...
class Merchant(QWidget):
    ...
    def __init__(self, username):
        ...
        self.dishes = QLabel(self)
        self.dishes.setPixmap(QPixmap("./image/dishes_abstract.jpg"))
        self.dishes.setScaledContents(True)
        self.dishes.resize(430, 430)
        self.dishes.move(460, 280)

        self.dishes_button = QPushButton("菜品管理", self)
        self.dishes_button.move(460, 710)
        self.dishes_button.resize(430, 60)
        self.dishes_button.setFont(QFont('幼圆', 13))
        self.dishes_button.clicked.connect(self.func_dishes_button)
        ...
        self.contract.setPixmap(QPixmap("./image/default_rider.jpg"))
        self.contract.setScaledContents(True)
        self.contract.resize(430, 430)
        self.contract.move(460, 810)

        self.contract_button = QPushButton("签约骑手", self)
        ...
        self.main_items += [self.dishes, self.dishes_button]
        ...
        self.personal_portrait.setPixmap(QPixmap(self.info[14]))
        ...
        self.change_name = QPushButton("修改店名", self)
        self.change_name.resize(180, 50)
        self.change_name.setFont(QFont("宋体", 12))
        self.change_name.move(650, 305)
        self.change_name.clicked.connect(self.change_name_dialog)
        ...
        self.personal_items.append(self.change_name)
    ...
    def portrait_file_dialog(self):
        ...
        if file_name != "":
            sql = "update merchant set `portrait` = '%s' where username = '%s'" % (file_name, self.info[0])
            try:
                self.cursor.execute(sql)
                self.connect.commit()
                self.personal_portrait.setPixmap(QPixmap(file_name))
                self.info[14] = file_name
            except:
    ...
    def confirm_change_address(self, x, y):
        sql1 = "update merchant set `longitude` = %d where `username` = '%s'" % (x, self.info[0])
        sql2 = "update merchant set `latitude` = %d where `username` = '%s'" % (y, self.info[0])
        try:
    ...
    def change_name_dialog(self):
        ...
        input_name.setPlaceholderText("输入新店名")
        ...
        confirm.setStyleSheet("background-color: rgb(51, 153, 255)")
        confirm.clicked.connect(
            lambda: self.confirm_change_name(input_name.text()))

        self.dialog.setWindowTitle("修改店名")
        self.dialog.setWindowModality(Qt.ApplicationModal)
        self.dialog.exec_()

    def confirm_change_name(self, name):
        if len(name) == 0:
            QMessageBox.critical(self, "饱了没", "名称不能为空!")
        else:
            try:
                sql = "update `merchant` set `name` = '%s' where `username` = '%s'" % (name, self.info[0])
                print(sql)
                self.cursor.execute(sql)
                self.connect.commit()
                self.info[9] = name
    ...
    def info_refresh(self):
        ...
        sql = "select * from merchant where username = '%s'" % self.username
        self.cursor.execute(sql)
        self.info += list(self.cursor.fetchone())[1:]
        sql = "select * from `order` where `merchant_name` = '%s'" % self.username
        self.cursor.execute(sql)
    ...
    def func_request_button(self):
        self.info_refresh()
        sql = "select * from `order` where `merchant_name` = '%s' and `state` = 0" % self.info[0]
        self.cursor.execute(sql)
    ...
    def create_request_item(self, request):
        ...
        sql = "select * from customer where `username` = '%s'" % request[6]
        self.cursor.execute(sql)
        ...
        money = QLabel(self)
        sql = "select sumMoney(%d)" % request[0]
        self.cursor.execute(sql)
        num = self.cursor.fetchone()[0]
        money.setText("订单金额: "+str(num)+"元")

        address = QLabel(self)
        address.setText("送达地址:(%d, %d) " % (cust_info[7], cust_info[8]))
        ...
        accept_button.clicked.connect(lambda: self.accept_order(accept_button, request[0]))
    ...
    def accept_order(self, btn, order_id):
        try:
            sql1 = "update `order` set `state` = 1 where `order_id` = %d" % order_id
            sql2 = "update `order` set `merchant_accept_time` = '%s' where `order_id` = %d" % (util.get_time(), order_id)
            self.cursor.execute(sql1)
            self.cursor.execute(sql2)
            self.connect.commit()
    ...
    def func_dishes_button(self):
        self.info_refresh()
        sql = "select * from `product` where `master` = '%s'" % self.info[0]
        self.cursor.execute(sql)
        all_dishes = self.cursor.fetchall()

        self.dialog = QDialog()
        self.dialog.setFixedSize(900, 900)
        self.dialog.setWindowIcon(QIcon("./image/icon.jpg"))
        self.dialog.setWindowTitle("菜品管理")
        self.dialog.setWindowModality(Qt.ApplicationModal)
        self.dialog.setPalette(self.login_palette)

        dishes_list = QScrollArea(self.dialog)
        dishes_list.move(0, 0)
        dishes_list.resize(900, 900)
        items = QWidget()
        vLayout = QVBoxLayout(items)
        for dish in all_dishes:
            vLayout.addWidget(self.create_dish_item(dish))
        dishes_list.setWidget(items)
        self.dialog.exec_()

    def create_dish_item(self, dish):
        groupBox = QGroupBox(self)
        pic = QLabel(self)
        pic.setPixmap(QPixmap(dish[3]))
        pic.setScaledContents(True)
        pic.resize(150, 150)
        pic.move(0, 0)
        ...
        name.setText(tmp)
        price = QLabel("价格:"+str(dish[1])+"元", self)
        num = QLabel("剩余:"+str(dish[-1]), self)
        sale = QLabel("销量:"+str(dish[2]), self)

        text_layout = QVBoxLayout()
        text_layout.addWidget(name)
        text_layout.addWidget(price)
        text_layout.addWidget(num)
        text_layout.addWidget(sale)
        text_widget = QWidget()
        text_widget.setLayout(text_layout)

        change_name_button = QPushButton("修改名称", self)
        change_name_button.clicked.connect(lambda: self.change_dish_name(name, dish[0]))
        change_price_button = QPushButton("修改价格", self)
        change_price_button.clicked.connect(lambda: self.change_dish_price(price, dish[0]))
        change_num_button = QPushButton("修改库存", self)
        change_num_button.clicked.connect(lambda: self.change_dish_num(num, dish[0]))
        delete_dish_button = QPushButton("删除菜品", self)
        delete_dish_button.setStyleSheet("background-color: rgb(238, 44, 44)")
        delete_dish_button.clicked.connect(lambda: self.delete_dish(delete_dish_button, dish[0]))

        button_layout = QVBoxLayout()
        button_layout.addWidget(change_name_button)
        button_layout.addWidget(change_price_button)
        button_layout.addWidget(change_num_button)
        button_layout.addWidget(delete_dish_button)
        button_widget = QWidget()
        button_widget.setLayout(button_layout)
    ...
    def func_evaluate_button(self):
        ...
        sql = "select `order_id`,`customer_name`,`score`,`comment` " \
              ...
              "on `order`.`merchant_evaluate_id` = `evaluate`.`evaluate_id` " \
              "where `order`.`merchant_name` = '%s'" % self.info[0]
        self.cursor.execute(sql)
    ...
    def pull_apply(self):
        ...
        input_name.setFont(QFont("宋体", 12))
        input_name.resize(450, 50)
        input_name.setPlaceholderText("输入骑手ID")
        input_name.move(25, 20)
        input_name.setMaxLength(50)
    ...
    def confirm_pull_apply(self, name):
        if len(name) == 0:
            ...
            sql = "select count(*) from `users` where `username` = '%s' and `user_type` = 'rider'" % name
            self.cursor.execute(sql)
            if self.cursor.fetchone()[0] == 0:
                QMessageBox.critical(self, "饱了没", "该骑手不存在!")
            else:
                sql = "select count(*) from `apply` where `rider_name` = '%s' " \
                      "and `merchant_name` = '%s'" % (name, self.info[0])
                self.cursor.execute(sql)
                if self.cursor.fetchone()[0] != 0:
                ...
                if self.cursor.fetchone()[0] != 0:
                    ...
                    return
                try:
                    sql = "insert into `apply` values('%s','%s', 0)" % (name, self.info[0])
                    self.cursor.execute(sql)
                    self.connect.commit()
    ...
    def func_contract_button(self):
        self.info_refresh()
        sql = "select * from `apply` where `merchant_name` = '%s' and `direction` = 1" % self.info[0]
        self.cursor.execute(sql)
        all_apply = self.cursor.fetchall()
        self.dialog = QDialog()
        self.dialog.setFixedSize(1400, 1400)
        self.dialog.setWindowIcon(QIcon("./image/icon.jpg"))
        self.dialog.setWindowTitle("签约骑手")
        self.dialog.setWindowModality(Qt.ApplicationModal)
        self.dialog.setPalette(self.login_palette)
    ...
    def create_apply_item(self, apply):
        groupBox = QGroupBox(self)
        sql = "select * from rider where `username` = '%s'" % apply[0]
        self.cursor.execute(sql)
        rider_info = self.cursor.fetchone()
        pic = QLabel(self)
        pic.setPixmap(QPixmap(rider_info[2]))
        pic.setScaledContents(True)
        pic.resize(50, 50)
        pic.move(0, 0)

        name = QLabel(self)
        name.setText("申请骑手: " + rider_info[0])

        sum_order = QLabel(self)
        sum_order.setText("骑手总单数: " + str(rider_info[4]))

        score = QLabel(self)
        if rider_info[8] == 0: score.setText("骑手评分: 0.0")
        else: score.setText("骑手评分: %.1f" % (rider_info[5]/rider_info[8]))

        address = QLabel(self)
        address.setText("骑手地址: (%d, %d)" % (rider_info[6], rider_info[7]))
        ...
        accept_button = QPushButton("接受", self)
        accept_button.setStyleSheet("background-color: rgb(51, 153, 255)")
        accept_button.clicked.connect(lambda: self.accept_contract(accept_button, rider_info[0]))
    ...
    def accept_contract(self, btn, name):
        try:
            sql1 = "delete from `apply` " \
                   "where `rider_name` = '%s'" \
                   "and `merchant_name` = '%s'" % (name, self.info[0])
            sql2 = "insert into `contract` values('%s','%s')" % (name, self.info[0])
            self.cursor.execute(sql1)
            self.cursor.execute(sql2)
            self.connect.commit()
            btn.setText("已接受")
    ...
    def paint_personal(self, event, qp):
        ...
        qp.drawText(350, 340, "店名:"+self.info[9])
        qp.drawText(350, 435, "注册:" + self.info[5].strftime('%Y-%m-%d'))
        qp.drawText(70, 540, "邮箱: " + self.info[4])
        qp.drawText(70, 645, "电话: " + self.info[3])
        qp.drawText(70, 750, "收入: " + str(float(self.info[17])) + "元")
        if self.info[16] == 0: qp.drawText(70, 855, "评分: 0.0")
        else: qp.drawText(70, 855, "评分: %.1f" % (self.info[8]/self.info[16]))
        qp.drawText(70, 960, "总订单数: " + str(int(self.info[7])))
        qp.drawText(70, 1065, "地址: (%d, %d)" % (int(self.info[10]), int(self.info[11])))


if __name__ == '__main__':
    app = QApplication(sys.argv)
    win = Merchant("merchant")
    win.show()
    sys.exit(app.exec_())
    pass
